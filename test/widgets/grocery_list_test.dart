...
void main() {
  group('GroceryList Widget Tests', () {
    ...
    testWidgets('should display all grocery items from mock data', (
      WidgetTester tester,
    ) async {
      // Act
      await tester.pumpWidget(const MaterialApp(home: GroceryList()));
      ...
      expect(
        find.text('1'),
        findsNWidgets(2),
      ); // Milk and Beef Steak both have quantity 1
      expect(find.text('5'), findsOneWidget); // Bananas have quantity 5
    });

    testWidgets('should display correct number of grocery item tiles', (
      WidgetTester tester,
    ) async {
      // Act
      await tester.pumpWidget(const MaterialApp(home: GroceryList()));
      ...
      // Assert
      expect(
        find.byType(GroceryItemTile),
        findsNWidgets(3),
      ); // Should have 3 items from mock data
    });

    testWidgets('should have a scrollable list view', (
      WidgetTester tester,
    ) async {
      // Act
      await tester.pumpWidget(const MaterialApp(home: GroceryList()));
    ...
    testWidgets('should handle empty state if no items', (
      WidgetTester tester,
    ) async {
